...
function fallbackReturnUrl(params: Record<string, string | undefined>): string {
  const url = new URL(typeof window !== "undefined" ? window.location.origin : "https://docfacil.com.br");
    ...
    const poll = async () => {
      ...
      try {
        ...
        if (attempts >= PAYMENT_POLL_MAX_ATTEMPTS) {
          console.error("[PixPaymentView] Falha ao consultar status do pagamento:", error);
      ...
      <div className="mx-auto max-w-2xl px-4 sm:px-6 py-8 sm:py-12">
        ...
        <div className="text-center mb-7">
          ...
          <p className="mt-2 text-ink/60 max-w-lg mx-auto">
            Assim que você concluir a transferência no aplicativo do seu banco, seu documento será liberado automaticamente.
          </p>
        </div>

        <div className="rounded-3xl border border-[var(--border)] bg-surface shadow-md overflow-hidden">
              ...
              <div className="space-y-7">
                ...
                <div>
                  ...
                  <button
                    ...
                    disabled={!pix?.brCode}
                    className="mt-3 w-full inline-flex items-center justify-center gap-2 h-12 rounded-xl bg-[var(--blue-royal)] text-white font-bold hover:bg-[#1e44a8] transition-colors disabled:opacity-50 cursor-pointer"
                  >
  ...
  return (
    <div className="py-10 text-center">
      ...
      <button
        type="button"
        onClick={onAction}
        className="mt-6 inline-flex items-center justify-center h-11 px-5 rounded-xl bg-[var(--blue-royal)] text-white font-semibold cursor-pointer"
      >
        {actionLabel}
      </button>
    </div>
    ...
    <section className="pt-[72px] min-h-[70vh] grid place-items-center bg-paper px-4">
      <div className="max-w-md text-center rounded-2xl border border-[var(--border)] bg-surface p-8">
        ...
        <button
          type="button"
          onClick={onBack}
          className="mt-5 inline-flex items-center justify-center h-11 px-5 rounded-xl bg-[var(--blue-royal)] text-white font-semibold cursor-pointer"
        >
          Voltar ao checkout
        </button>
      </div>
